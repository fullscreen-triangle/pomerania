/**
 * The four-column relaxation, ported from
 * four-sided-triangle/equilateral/src/lib/relaxation.ts. Central columns
 * (a, b) always converge toward each other each round. Provoked columns
 * (pa, pb) converge too UNLESS `freezeProvoked` is set — a false-friend
 * demo freezes them so central agreement is reached while the provoked
 * follow-ups stay permanently apart, which is exactly the failure a
 * central-only comparison misses. A genuine-agreement pair leaves
 * `freezeProvoked` unset, so all four columns can actually reach
 * quiescence together.
 */
